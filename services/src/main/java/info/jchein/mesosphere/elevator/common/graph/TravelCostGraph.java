...
public class TravelCostGraph<V, E>
implements ITravelCostGraph<V, E>
{
   private DefaultDirectedWeightedGraph<V, E> costGraph;


   // =10-((N3)/(1.9*$M$2))^3.6
   TravelCostGraph( Collection<V> vertices, BiFunction<V, V, Double> costFunction,
      EdgeFactory<V, E> edgeFactory )
   {
      GraphBuilder<V, E, ? extends DefaultDirectedWeightedGraph<V, E>> builder =
         DefaultDirectedWeightedGraph.<V, E> createBuilder(edgeFactory);
         ...
         .<Object[]> reduce((Object[]) null, (Object[] result, V nextVertex) -> {
            ...
            if (result == null) {
               ...
               retVal =
                  new Object[]
                  {
                     ((Double) result[0]) + this.getTraversalCost((V) result[2], nextVertex), result[1],
                     nextVertex
               };
            }
            ...
            if (left == null) {
               ...
               retVal =
                  ...
                  {
                     ((Double) left[0]) +
                        ((Double) right[0]) + this.getTraversalCost((V) left[2], (V) right[1]),
                     left[1], right[2]
               };
            }
