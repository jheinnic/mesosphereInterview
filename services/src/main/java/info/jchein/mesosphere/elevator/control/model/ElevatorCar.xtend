...
class ElevatorCar implements IElevatorCar, IElevatorCarPort {
	...
	@Autowired
	new(
		@NotNull IRuntimeClock clock, @NotNull IRuntimeEventBus eventBus, @NotNull ElevatorGroupBootstrap bootstrapData
	) {
		this.carIndex = ID_SEQUENCE.incrementAndGet();
		this.numFloors = bootstrapData.building.numFloors;
		this.clock = clock;
	...
	def void attachDriver(IElevatorCarDriver driver)
	{
		...
		val InitialElevatorCarState data = driver.initialize();
