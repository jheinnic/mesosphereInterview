...
public class RuntimeScheduler implements IRuntimeScheduler
{
   ...
   extends ScheduledCallback<T>
   {
      ...
      {
         RuntimeScheduler.this.interruptHeap.insert(this.node, timeNow + this.nextExpectedInterval);
      }
   }
   ...
   implements Action0
   {
      ...
      {
         this.timeZero = RuntimeScheduler.this.worker.now();
         this.timeNext = this.timeZero + RuntimeScheduler.this.tickMillis;
      }

      @Override
      public void call()
      {
         final long timeNow = RuntimeScheduler.this.worker.now();
         // final long deltaNow = timeNow - this.timeZero;

         final long nextTimeNext = this.timeNext + RuntimeScheduler.this.tickMillis;
         final long deltaNext = nextTimeNext - timeNow;
         final long clockSkew = deltaNext - RuntimeScheduler.this.tickMillis;
         ...
         FibonacciHeapNode<ScheduledCallback<?>> closestEvent = RuntimeScheduler.this.interruptHeap.min();
         while (closestEvent.getKey() < this.timeNext) {
            ...
            closestEvent = RuntimeScheduler.this.interruptHeap.removeMin();
            ...
            closestEvent = RuntimeScheduler.this.interruptHeap.min();
         }
   ...
   @Autowired
   public RuntimeScheduler(@NotNull Scheduler scheduler, @NotNull SystemRuntimeProperties systemProps)
   {
