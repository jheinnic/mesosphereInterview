...
public class Snippet2 {
	public static void main(String[] args) throws InterruptedException {
		...
		Observable<String> x1 = Observable.interval(2, TimeUnit.MILLISECONDS, myScheduler25)
				.map(r -> String.format("x1(%d)", 1+r)).take(100);
		Observable<String> x5 = Observable.interval(10, TimeUnit.MILLISECONDS, myScheduler25)
				.map(r -> String.format("x5(%d)", 1+r)).take(20);
		Observable<String> x7 = Observable.interval(14, TimeUnit.MILLISECONDS, myScheduler25)
				.map(r -> String.format("x7(%d)", 1+r)).take(14);
		Observable<String> x25 = Observable.interval(50, TimeUnit.MILLISECONDS, myScheduler25)
				.map(r -> String.format("x25(%d)", 1+r)).take(4);
		Observable<String> x40 = Observable.interval(80, TimeUnit.MILLISECONDS, myScheduler25)
...
//			.doOnNext(v -> { System.out.println( String.format("X %s", v)); })
			.buffer(2, TimeUnit.MILLISECONDS, myScheduler2)
//			.skip(1)
//			.observeOn(Schedulers.immediate())
			.map(r -> r.parallelStream().map(
				intv -> Long.toString(intv.getIntervalInMilliseconds()) + "-" + intv.getValue()
