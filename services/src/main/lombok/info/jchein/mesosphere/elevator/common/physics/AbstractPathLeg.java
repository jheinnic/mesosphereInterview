...
import info.jchein.mesosphere.elevator.common.physics.AbstractPathLegMomentIterator;
...
implements IPathLeg
{
   private static final Interner<AbstractPathLeg> INTERN_CACHE = Interners.newWeakInterner();
    ...
    */
   abstract AbstractPathLegMomentIterator getMomentIterator(double firstTickDuration, double tickDuration);

   abstract AbstractPathLeg doTruncate(double newInitialTime);

   public AbstractPathLeg truncate(double newInitialTime) {
      final AbstractPathLeg retVal = this.doTruncate(newInitialTime);
   ...
   public Iterator<PathMoment> toMomentIterator(final double firstTick, final double tickDuration)
   {
      ...
      if (initialTime == firstTick) {
         ...
         // sequence to be returned. Instead, we leverage a feature of the AbstractPathLegMomentIterator that allows it
         // to use a different tickDuration for the first value returned, after which it performs a one-time update to
         // the tickDuration it uses for the remainder of its work.
